...
describe('make a simple order forms and should', () => {
  ...
  it('work with out problems', () => {
    cy.mount(<TestOrderForm company={testCompany} />)
    cy.contains('Nueva orden').click()
    cy.get('input[name="name"]').type('test name')
    cy.contains('li', 'test name').click()
    cy.get('input[name="client.phone"]').should('have.value', '+52 555 555 555')
    cy.contains('button', 'staff name').click()
    cy.contains('Calendario de entregas: staff name')
  })
})
