...
const searchValueInObject = (
  ...
  // Si hemos alcanzado la profundidad máxima, terminamos la búsqueda
  if (depth > maxDepth) {
    return false
  }
  ...
  for (const prop in obj) {
    ...
    if (typeof obj[prop] === 'object') {
      ...
      if (Array.isArray(obj[prop])) {
        ...
        for (const item of obj[prop]) {
          // Llama recursivamente a la función para buscar en el elemento del array
          if (searchValueInObject(item, search, maxDepth, depth + 1)) {
            return true
          }
        }
      } else {
        // Llama recursivamente a la función para buscar en el objeto interno
        if (searchValueInObject(obj[prop], search, maxDepth, depth + 1)) {
          return true
        }
      }
    } else {
